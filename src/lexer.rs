...
fn lex_line(line: &str, line_no: usize, out: &mut Vec<Token>) {
    ...
    if line == "} else {" {
        ...
        out.push(Token {
            kind: TokenKind::Ident("else".to_string()),
            line: line_no,
        });
        out.push(Token {
            kind: TokenKind::LBrace,
...
fn lex_non_brace_line(line: &str, line_no: usize, out: &mut Vec<Token>) {
    if line.starts_with('@') {
        ...
        if !val.is_empty() {
            out.push(Token {
                kind: TokenKind::RawValue(val.to_string()),
                line: line_no,
            });
        }
        return;
    ...
    if line.starts_with("else if ") {
        ...
        out.push(Token {
            kind: TokenKind::Ident("if".to_string()),
            line: line_no,
        });
    ...
    if rest.is_empty() && is_ident(first) {
        out.push(Token {
            kind: TokenKind::Ident(first.to_string()),
            line: line_no,
        });
        return;
    }
    ...
    if !cmd.trim().is_empty() {
        out.push(Token {
            kind: TokenKind::RawValue(cmd.trim().to_string()),
            line: line_no,
        });
    }
}
...
fn lex_condition_expr(expr: &str, line_no: usize, out: &mut Vec<Token>) {
    if let Some(idx) = expr.find("==") {
        ...
        out.push(Token {
            kind: TokenKind::RawValue(lhs.to_string()),
            line: line_no,
        });
        out.push(Token {
            kind: TokenKind::EqEq,
        ...
        out.push(Token {
            kind: TokenKind::RawValue(rhs.to_string()),
            line: line_no,
        });
        ...
        out.push(Token {
            kind: TokenKind::RawValue(expr.to_string()),
            line: line_no,
        });
    }
}
